...
function parseAtisRunways(string $atisText): array {
    ...
    // Pattern helpers
    $rwyNum = '([0-3]?\d[LRC]?)';
    $rwyList = '([0-3]?\d[LRC]?(?:\s*(?:AND|,|\/)\s*[0-3]?\d?[LRC]?)*)';
    ...
    if (preg_match_all('/(?:DEP(?:ART(?:ING|URE)?)?|DEPTG|DEPG|DPTG|DEPARTING)\s+(?:RWYS?\s+)?'.$rwyList.'/i', $text, $m)) {
        foreach ($m[1] as $rwyText) {
    ...
    if (preg_match_all('/DEPARTURE\s+RUNWAY\s+'.$rwyNum.'/i', $text, $m)) {
        foreach ($m[1] as $rwy) {
...
function extractRunwayNumbers(string $text): array {
    ...
    foreach ($parts as $part) {
        ...
        if (preg_match('/^([0-3]?\d)\s*(L(?:EFT)?|R(?:IGHT)?|C(?:ENTER)?)?$/', $part, $m)) {
            $runway = normalizeRunway($m[1] . ($m[2] ?? ''));
            $runways[] = $runway;
            $lastNumber = $m[1];
        } elseif ($lastNumber && preg_match('/^(L(?:EFT)?|R(?:IGHT)?|C(?:ENTER)?)$/', $part, $m)) {
