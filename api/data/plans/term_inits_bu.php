                ...
                echo '<tr class="bg-dark text-light">';
                    if ($perm == true) {
                        ...
                        echo '<td class="text-center" colspan="5">';
                            echo '<a href="javascript:void(0)" data-toggle="tooltip" title="Edit Terminal Initiative"><span class="badge badge-warning" data-toggle="modal" data-target="#editterminalinitModal" data-id="'.intval($data['id']).'" data-title="'.htmlspecialchars($data['title'], ENT_QUOTES, 'UTF-8').'" data-context="'.htmlspecialchars($data['context'], ENT_QUOTES, 'UTF-8').'">
                            <i class="fas fa-pencil-alt"></i> Edit</span></a>';
                            echo ' ';
                            echo '<a href="javascript:void(0)" onclick="deleteTerminalInit('.intval($data['id']).')" data-toggle="tooltip" title="Delete Terminal Initiative"><span class="badge badge-danger"><i class="fas fa-times"></i> Delete</span></a>';
                        echo '</td>';
                    ...
                    foreach ($times as &$time) {
                        ...
                        if ($t_q['total'] > 0) {
                            if ($t_q['probability'] == 0) {
                                // CDW
                                if ($perm == true) {
                                    echo '<td style="width: 3%; background-color: #ffd501;" onclick="changeTermTime(`'.intval($t_q['id']).'`, `'.intval($t_q['probability']).'`)" data-toggle="tooltip" title="Criticial Decision Window"></td>';
                                } else {
                                ...
                                // Possible
                                if ($perm == true) {
                                    echo '<td style="width: 3%;" onclick="changeTermTime(`'.intval($t_q['id']).'`, `'.intval($t_q['probability']).'`)" class="bg-info" data-toggle="tooltip" title="Possible"></td>';
                                } else {
                                ...
                                // Probable
                                if ($perm == true) {
                                    echo '<td style="width: 3%; background-color: #ff5601;" onclick="changeTermTime(`'.intval($t_q['id']).'`, `'.intval($t_q['probability']).'`)" data-toggle="tooltip" title="Probable"></td>';
                                } else {
                                ...
                                // Expected
                                if ($perm == true) {
                                    echo '<td style="width: 3%; background-color: #ff012b;" onclick="changeTermTime(`'.intval($t_q['id']).'`, `'.intval($t_q['probability']).'`)" data-toggle="tooltip" title="Expected"></td>';
                                } else {
                                ...
                                // Actual
                                if ($perm == true) {
                                    echo '<td style="width: 3%; background-color: #4400CD;" onclick="deleteTermTime(`'.intval($t_q['id']).'`)" data-toggle="tooltip" title="Actual"></td>';
                                } else {
