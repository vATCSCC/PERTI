 ...
 * 
 * Returns flight data from the normalized ADL tables in unified flight record format.
 * Uses JOINs across: adl_flight_core, adl_flight_position, adl_flight_plan, 
 *                    adl_flight_times, adl_flight_tmi, adl_flight_aircraft
 * 
...
// Main query using normalized ADL tables
// Column names VERIFIED against schema 2026-01-15
$sql = "
    SELECT 
        -- adl_flight_core
...
while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
    $flights[] = formatFlightRecord($row);
}
sqlsrv_free_stmt($stmt);

SwimResponse::paginated($flights, $total, $page, $per_page);

function formatFlightRecord($row) {
    ...
    return [
        ...
        'times' => [
            ...
            'etd_runway' => formatDT($row['etd_runway_utc']),
