...
(function() {
    ...
    async function saveReroute() {
        ...
        try {
            ...
            showToast(`Reroute ${data.action}`, 'success');
        } catch (e) {
    ...
    async function loadReroutesList() {
        ...
        try {
            ...
            if (!data.reroutes?.length) {
                tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted">No reroutes</td></tr>';
                return;
            }
    ...
    window.overrideCompliance = async function(flightId, callsign) {
        const status = prompt(`Override ${callsign}:\nCOMPLIANT, PARTIAL, NON_COMPLIANT, EXEMPT`);
            ...
            return `<tr class="${cls}">
                ...
                <td>
                    <div class="btn-group btn-group-sm">
                        ...
                        ${isAssigned ? `<button class="btn btn-outline-warning" onclick="overrideCompliance(${f.id},'${f.callsign}')" title="Override"><i class="fas fa-edit"></i></button>
                        <button class="btn btn-outline-secondary" onclick="showHistory(${f.id})" title="History"><i class="fas fa-history"></i></button>` : ''}
                    </div>
                </td>
            </tr>`;
        }).join('');
    ...
    function renderExemptTable(flights) {
        ...
        if (!flights?.length) {
            tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-4">No exempt flights</td></tr>';
            return;
        }
    ...
    function clearFlightTable() {
        document.getElementById('rr_flight_table_body').innerHTML = '<tr><td colspan="10" class="text-center text-muted py-4">Click "Preview"</td></tr>';
        document.getElementById('rr_exempt_table_body').innerHTML = '<tr><td colspan="4" class="text-center text-muted">No exempt</td></tr>';
        ...
        flights.forEach(f => {
            ...
            if (lat && lon) {
                ...
                L.circleMarker([lat, lon], { radius: 6, fillColor: color, color: '#fff', weight: 1, fillOpacity: 0.9 })
                    .bindPopup(`<strong>${f.callsign}</strong><br>${f.fp_dept_icao || f.dep_icao} → ${f.fp_dest_icao || f.dest_icao}<br>Status: ${s}`)
    ...
    window.showFlightDetail = function(key) {
        ...
        document.getElementById('flightDetailBody').innerHTML = `
            <table class="table table-sm">
                <tr><th>Key</th><td>${f.flight_key}</td></tr>
                <tr><th>Route</th><td>${f.fp_dept_icao || f.dep_icao} → ${f.fp_dest_icao || f.dest_icao}</td></tr>
                <tr><th>Aircraft</th><td>${f.aircraft_type || f.ac_type || '--'}</td></tr>
                <tr><th>Altitude</th><td>FL${Math.round((f.fp_altitude_ft || f.filed_altitude || 0) / 100)}</td></tr>
                <tr><th>Filed</th><td><small>${f.fp_route || f.route_at_assign || '--'}</small></td></tr>
                <tr><th>Current</th><td><small>${f.current_route || '--'}</small></td></tr>
                <tr><th>Status</th><td><span class="badge ${STATUS_CLASSES[f.compliance_status] || 'badge-secondary'}">${f.compliance_status || '--'}</span> ${f.compliance_pct != null ? f.compliance_pct + '%' : ''}</td></tr>
                ${f.protected_fixes_crossed ? `<tr><th>Protected ✓</th><td class="text-success">${f.protected_fixes_crossed}</td></tr>` : ''}
                ${f.avoid_fixes_crossed ? `<tr><th>Avoid ✗</th><td class="text-danger">${f.avoid_fixes_crossed}</td></tr>` : ''}
            </table>`;
