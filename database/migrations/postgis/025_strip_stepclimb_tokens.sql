-- Migration 025: Strip SimBrief step climb tokens from route parsing
--
-- Problem: SimBrief-generated routes include speed/altitude change tokens
-- attached to fixes via slash (e.g., ERGOM/N0460F380, TABIL/M084F350).
-- The slash-splitting preprocessor splits these into two tokens:
--   ERGOM/N0460F380 → ERGOM + N0460F380
-- The N0460F380 part is then added to the processed token list, which:
--   1. Breaks airway expansion (N0460F380 sits between fix and airway)
--   2. Fails to resolve as a waypoint (no fix named N0460F380)
--   3. Causes gaps in the expanded route
--
-- Fix: Skip tokens matching ICAO speed/altitude format in two places:
--   1. After slash-splitting: discard speed/altitude suffix parts
--   2. In main loop: skip standalone speed/altitude tokens
--
-- Pattern: ^[NMK]\d{3,4}[FSA]\d{3,4}$
--   N0460F360 = 460 kts TAS, FL360
--   M084F350  = Mach 0.84, FL350
--   K0900S1060 = 900 km/h, 10600m
--   N0460A100 = 460 kts, 10000 ft
--
-- Impact: Routes with SimBrief step climbs now parse correctly.
-- No false positives: no real fix or airway matches this pattern.
--
-- Date: 2026-04-23
-- Depends on: migration 024 (airway suffix detection)
...
BEGIN
    ...
    -- Also strips SimBrief step climb speed/altitude tokens (migration 025)
    ...
    FOR v_i IN 1..array_length(v_raw_parts, 1) LOOP
        ...
        -- Skip standalone speed/altitude tokens (SimBrief step climbs)
        -- Pattern: N0460F360, M084F350, K0900S1060, N0460A100
        IF v_part ~ '^[NMK]\d{3,4}[FSA]\d{3,4}$' THEN
            CONTINUE;
        END IF;
        ...
        ELSIF position('/' IN v_part) > 0 THEN
            ...
            FOR v_j IN 1..array_length(v_slash_parts, 1) LOOP
                IF v_slash_parts[v_j] IS NOT NULL AND v_slash_parts[v_j] != '' THEN
                    -- Skip runway suffixes (e.g., 07L, 31R|31L)
                    IF v_j > 1 AND v_slash_parts[v_j] ~ '^\d{2}[LRCB]?(\|\d{2}[LRCB]?)*$' THEN
                        NULL;
                    -- Skip speed/altitude suffixes (SimBrief step climbs: N0460F360, M084F350)
                    ELSIF v_slash_parts[v_j] ~ '^[NMK]\d{3,4}[FSA]\d{3,4}$' THEN
                        NULL;
                    ELSE
...
COMMENT ON FUNCTION expand_route(TEXT) IS
    'Migration 025: Strip SimBrief step climb speed/altitude tokens from route '
    'parsing. Tokens like N0460F360, M084F350 (ICAO speed/FL format) are now '
    'discarded during preprocessing instead of being passed to waypoint '
    'resolution where they break airway expansion. Preserves all features '
    'from migrations 020-024.';


-- ============================================================================
-- Verification queries (run manually after applying)
-- ============================================================================
-- Step climb with slash (the bug this fixes):
-- SELECT * FROM expand_route('KJFK DEEZZ6.CANDR ERGOM/N0460F380 A602 TABIL KLAX');
-- Expected: KJFK → procedure waypoints → ERGOM → A602 waypoints → TABIL → KLAX
-- (N0460F380 should NOT appear, and A602 should expand correctly)
--
-- Standalone step climb token:
-- SELECT * FROM expand_route('KJFK DEEZZ6.CANDR ERGOM N0460F380 A602 TABIL KLAX');
-- Expected: Same as above (standalone N0460F380 also stripped)
--
-- Mach step climb:
-- SELECT * FROM expand_route('KJFK CANDR M084F350 J60 BOSCO KLAX');
-- Expected: M084F350 stripped, J60 expands between CANDR and BOSCO
--
-- Regression: Standard routes (no step climbs):
-- SELECT * FROM expand_route('KJFK DEEZZ6.CANDR J60 KBOS');
-- Expected: Same as migration 024
--
-- Regression: Letter-suffix airways still work:
