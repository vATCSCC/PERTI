    ...
    if (isset($results['reroute_results'])) {
        foreach ($results['reroute_results'] as $key => $r) {
            $formatted['reroute_results'][] = [
                ...
                'destinations' => $r['destinations'] ?? [],
                'total_flights' => $r['total_flights'] ?? 0,
                'filed_compliance_pct' => array_key_exists('filed_compliance_pct', $r) ? $r['filed_compliance_pct'] : 0,
                'flown_compliance_pct' => array_key_exists('flown_compliance_pct', $r) ? $r['flown_compliance_pct'] : 0,
                'note' => $r['note'] ?? null,
