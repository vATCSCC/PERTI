-- ============================================================================
-- Migration: FIR Pattern Exclusions + SP Update
--
-- Creates a config table for FIR pattern exclusions (e.g., EGYP excluded from
-- EG* pattern) and updates sp_GS_Model to respect them.
--
-- The exclusion table is general-purpose: any FIR prefix can declare excluded
-- ICAO prefixes, allowing future additions without SP changes.
--
-- Date: 2026-03-11
-- ============================================================================
...
-- Step 1: Create exclusion config table
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'fir_pattern_exclusions')
BEGIN
    CREATE TABLE dbo.fir_pattern_exclusions (
        exclusion_id    INT IDENTITY(1,1) PRIMARY KEY,
        fir_pattern     NVARCHAR(8)   NOT NULL,   -- ICAO prefix, e.g. 'EG'
        excluded_prefix NVARCHAR(8)   NOT NULL,   -- Excluded ICAO prefix, e.g. 'EGYP'
        reason          NVARCHAR(128) NULL,        -- Human-readable reason
        CONSTRAINT UQ_fir_exclusion UNIQUE (fir_pattern, excluded_prefix)
    );

    PRINT 'Created dbo.fir_pattern_exclusions table';
END
ELSE
    PRINT 'dbo.fir_pattern_exclusions already exists — skipping CREATE';
GO

-- Step 2: Seed exclusion data
IF NOT EXISTS (
    SELECT 1 FROM dbo.fir_pattern_exclusions
    WHERE fir_pattern = 'EG' AND excluded_prefix = 'EGYP'
)
BEGIN
    INSERT INTO dbo.fir_pattern_exclusions (fir_pattern, excluded_prefix, reason)
    VALUES ('EG', 'EGYP', 'UK overseas territory - Falkland Islands (RAF Mount Pleasant)');

    PRINT 'Seeded EG/EGYP exclusion';
END
ELSE
    PRINT 'EG/EGYP exclusion already exists — skipping seed';
GO

-- Step 3: Update sp_GS_Model to respect exclusions
...
BEGIN
    ...
    BEGIN
        ...
        SELECT UPPER(SUBSTRING(TRIM(value), 5, 8))
        FROM STRING_SPLIT(@dep_facilities, ' ')
    ...
    -- Load FIR exclusions from config table into table variable
    DECLARE @fir_exclusions TABLE (fir_pattern NVARCHAR(8), excluded_prefix NVARCHAR(8));

    IF @has_fir_patterns = 1
    BEGIN
        INSERT INTO @fir_exclusions (fir_pattern, excluded_prefix)
        SELECT e.fir_pattern, e.excluded_prefix
        FROM dbo.fir_pattern_exclusions e
        INNER JOIN @fir_patterns fp ON e.fir_pattern = fp.pattern;
    END
    ...
    flights_with_scope AS (
        SELECT
            ...
            CASE
                ...
                -- AND departure airport is not excluded by fir_pattern_exclusions config
                WHEN @has_fir_patterns = 1 AND EXISTS (
                    SELECT 1 FROM @fir_patterns fp
                    WHERE (cf.dep_fir LIKE fp.pattern + '%' OR cf.fp_dept_icao LIKE fp.pattern + '%')
                      AND NOT EXISTS (
                          SELECT 1 FROM @fir_exclusions ex
                          WHERE ex.fir_pattern = fp.pattern
                            AND cf.fp_dept_icao LIKE ex.excluded_prefix + '%'
                      )
                ) THEN 1
...
PRINT 'Updated sp_GS_Model with FIR pattern exclusion support';
PRINT 'Exclusions loaded from dbo.fir_pattern_exclusions config table';
PRINT 'Seed data: EG pattern excludes EGYP (Falkland Islands)';
GO
