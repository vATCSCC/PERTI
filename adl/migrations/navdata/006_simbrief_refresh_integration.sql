-- ============================================================================
-- Migration: SimBrief Parsing Integration into Refresh Cycle
-- 
-- Adds sp_ParseSimBriefDataBatch call to sp_Adl_RefreshFromVatsim_Normalized
-- 
-- Version: V8.5
-- Changes:
--   - Added Step 4c: SimBrief/ICAO Flight Plan Parsing
--   - Processes flights with unparsed SimBrief data each cycle
--   - Extracts: is_simbrief flag, step climbs, runway hints, ICAO Item 18
--   - Returns simbrief_parsed count in stats
--
-- Run Order: After 002_simbrief_parsing.sql
-- ============================================================================
...
PRINT '=== SimBrief Refresh Integration ===';
...
IF OBJECT_ID('dbo.sp_ParseSimBriefDataBatch', 'P') IS NULL
BEGIN
    RAISERROR('sp_ParseSimBriefDataBatch not found. Run 002_simbrief_parsing.sql first.', 16, 1);
    RETURN;
END
GO

IF OBJECT_ID('dbo.sp_Adl_RefreshFromVatsim_Normalized', 'P') IS NULL
BEGIN
    RAISERROR('sp_Adl_RefreshFromVatsim_Normalized not found.', 16, 1);
    RETURN;
END
GO
...
-- ============================================================================
-- Recreate the refresh procedure with SimBrief integration (V8.5)
-- ============================================================================
...
BEGIN
    ...
    WHEN MATCHED THEN
        UPDATE SET
            ...
            END,
            -- Reset SimBrief flag if route changed (V8.5)
            is_simbrief = CASE 
                WHEN target.fp_hash IS NULL OR target.fp_hash != source.route_hash 
                THEN NULL ELSE target.is_simbrief 
            END
    WHEN NOT MATCHED THEN
    ...
    ETDResolved AS (
        SELECT
            flight_uid, phase, deptime, deptime_minutes, fp_dof_utc, fp_enroute_minutes,
            CASE
    ...
    --
    -- Extracts from route/remarks:
    --   - is_simbrief flag (SimBrief detection)
    --   - Step climb waypoints with altitude/speed changes
    --   - Runway hints from SID/STAR names
    --   - ICAO Item 18 indicators (DOF, REG, PBN, etc.)
    --
    -- Only processes flights where is_simbrief IS NULL (unparsed)
    -- Limited batch size to avoid refresh cycle overrun
    -- ========================================================================
    
    IF OBJECT_ID('dbo.sp_ParseSimBriefDataBatch', 'P') IS NOT NULL
    BEGIN
        DECLARE @sb_result TABLE (
            flights_processed INT,
            simbrief_flights INT,
            flights_with_stepclimbs INT,
            elapsed_ms INT
        );
        
        INSERT INTO @sb_result
        EXEC dbo.sp_ParseSimBriefDataBatch 
            @batch_size = 50,      -- Process up to 50 flights per cycle
            @only_unparsed = 1;    -- Only flights where is_simbrief IS NULL
...
PRINT '';
PRINT '-- Check SimBrief detection status:';
PRINT 'SELECT is_simbrief, COUNT(*) AS flights';
PRINT 'FROM dbo.adl_flight_plan fp';
PRINT 'JOIN dbo.adl_flight_core c ON c.flight_uid = fp.flight_uid';
PRINT 'WHERE c.is_active = 1';
PRINT 'GROUP BY is_simbrief;';
PRINT '';
PRINT '-- Check step climbs:';
PRINT 'SELECT TOP 10 c.callsign, fp.stepclimb_count, fp.initial_alt_ft, fp.final_alt_ft';
PRINT 'FROM dbo.adl_flight_plan fp';
PRINT 'JOIN dbo.adl_flight_core c ON c.flight_uid = fp.flight_uid';
PRINT 'WHERE c.is_active = 1 AND fp.stepclimb_count > 0;';
PRINT '';
PRINT '-- Check runway extraction:';
PRINT 'SELECT TOP 10 c.callsign, fp.fp_dept_icao, fp.dep_runway, fp.fp_dest_icao, fp.arr_runway';
PRINT 'FROM dbo.adl_flight_plan fp';
PRINT 'JOIN dbo.adl_flight_core c ON c.flight_uid = fp.flight_uid';
PRINT 'WHERE c.is_active = 1 AND (fp.dep_runway IS NOT NULL OR fp.arr_runway IS NOT NULL);';
GO
