-- Migration 041: sp_Swim_BulkUpsert v4.0 — CTP Flow Event Tagging
-- Database: SWIM_API
-- Run as: jpeterson (DDL admin)
-- Changes from v3.0 (migration 031):
--   - Added flow_event_code (NVARCHAR(32)) to OPENJSON WITH clause (126 total)
--   - Added flow_gs_exempt (BIT) to OPENJSON WITH clause
--   - Added flow_event_code to row-hash (22 volatile columns)
--   - Added flow_event_code + flow_gs_exempt to MERGE SET + INSERT
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            j.*,
            -- Row hash on 22 key volatile columns (v4.0: +flow_event_code)
        ...
        FROM OPENJSON(@Json) WITH (
            ...
            -- TMI control (23 — v4.0: +flow_event_code, +flow_gs_exempt)
            gs_held             BIT,
        ...
        WHEN MATCHED AND (t.row_hash IS NULL OR t.row_hash <> s.row_hash) THEN UPDATE SET
            ...
            t.flow_event_code = s.flow_event_code,
            t.flow_gs_exempt = s.flow_gs_exempt,
            -- Aircraft
        ...
        WHEN NOT MATCHED BY TARGET THEN INSERT (
            ...
            flow_event_code, flow_gs_exempt,
            ...
            s.flow_event_code, s.flow_gs_exempt,
...
PRINT 'Created sp_Swim_BulkUpsert v4.0 (flow_event_code + flow_gs_exempt)';
GO
