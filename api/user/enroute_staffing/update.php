...
$stmt = $conn_sqli->prepare("UPDATE p_enroute_staffing SET staffing_status=?, staffing_quantity=?, comments=? WHERE id=?");
$stmt->bind_param("sssi", $staffing_status, $staffing_quantity, $comments, $id);

if ($stmt->execute()) {
    http_response_code(200);
} else {
    error_log("user/enroute_staffing/update error: " . $stmt->error);
