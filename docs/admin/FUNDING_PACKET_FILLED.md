...
- Implement alert routing (email + Discord (vATCSCC Server)) with a clear on-call escalation path.
...
KPIs:
- Ingest freshness SLO: Alert if no new writes > 2 minutes; monthly report maximum stall; target 99.9% of minutes have fresh ingest.
- MTTR for ingest failures: < 15 minutes.
- SQL monthly cost vs Jan-Feb 2026 baseline: 10-20% reduction (maintain peak headroom).
- Resolution KPI: Baseline and monthly report of downsampling/tiering usage; target 50% reduction in forced downscaling for peak-event operations while keeping costs governed.
- Data quality KPI: Baseline and monthly report of completeness/latency; target 20% reduction in missing/unknown fields and >80% multi-source correlation coverage for participating partner feeds.
...
Subject: White Paper Submission (NOFO 20-01): PERTI: A High-Resolution, Event-Scale Testbed for Flow Management Research Using Real-Time Network Operations Data
...
Program Area: Systems Science / Operations Research
NOFO: FAA-20-01
...
| Ray Foundation | https://rayfoundation.us/grant-requests/ | 2026-02-17 | Email inquiry sent to info@rayfoundation.us (fit + submission format) | Follow up on 2026-03-03 if no response | Jeremy Peterson |
