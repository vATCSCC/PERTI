<?php
/**
 * api/adl/demand/segment.php
 *
 * Route Segment Demand API - Returns flights passing through two fixes
 *
 * Unlike /airway which requires on_airway to be populated, this endpoint
 * finds any flights whose parsed route includes both fixes, regardless
 * of whether they filed via an airway or direct (DCT).
 *
 * Parameters:
...
if (!empty($missing)) {
    ...
    echo json_encode([
        ...
        "example" => "/api/adl/demand/segment?from_fix=CAM&to_fix=GONZZ&minutes=180"
    ]);
    exit;
}
...
// Execute query
$sql = "SELECT * FROM dbo.fn_RouteSegmentDemand(?, ?, ?, NULL) ORDER BY entry_eta";
$params = [$fromFix, $toFix, $minutes];
