...
$stmt = $conn_sqli->prepare("UPDATE p_terminal_staffing SET facility_name=?, staffing_status=?, staffing_quantity=?, comments=? WHERE id=?");
$stmt->bind_param("ssssi", $facility_name, $staffing_status, $staffing_quantity, $comments, $id);

if ($stmt->execute()) {
    http_response_code(200);
} else {
    error_log("terminal_staffing/update error: " . $stmt->error);
